...
export async function GET(req: NextRequest, context: { params: Promise<{ database: string }> }) {
    return withUserAndOrganizationHandler(async ({ userId, organizationId }) => {
        ...
        const sequences = hasMetadataCapability(metadata, 'getSequences') ? await metadata.getSequences(database) : [];

        return NextResponse.json(ResponseUtil.success(sequences));
    })(req);
}
