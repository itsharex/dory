...
import { PostgresTableStats } from '@/types/table-info';
...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string; database: string; table: string }> }) {
    return withUserAndTeamHandler(async ({ userId, teamId }) => {
        ...
        const errorMessages = {
            fallback: t('Api.Connection.Tables.Errors.StatsFailed'),
        ...
        try {
            ...
            if (!tableInfo?.postgresStats) {
                return NextResponse.json(
                    ResponseUtil.error({ code: ErrorCodes.INVALID_PARAMS, message: errorMessages.fallback }),
                    { status: 400 },
                );
            }
            const stats = await tableInfo.postgresStats(database, table);
            return NextResponse.json(ResponseUtil.success<PostgresTableStats | null>(stats ?? null));
        } catch (error) {
