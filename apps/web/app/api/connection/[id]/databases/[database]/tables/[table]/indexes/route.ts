...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string; database: string; table: string }> }) {
    return withUserAndOrganizationHandler(async ({ userId, organizationId }) => {
        ...
        const errorMessages = {
            fallback: t('Api.Connection.Tables.Errors.PropertiesFailed'),
        ...
        try {
            ...
            const indexes = await tableInfo.indexes(database, table);
            return NextResponse.json(ResponseUtil.success<TableIndexInfo[]>(indexes));
        } catch (error) {
            console.log('Error in GET table indexes route:', error);
