...
function extractActionsEvent(envelope: CopilotEnvelopeV1 | null, overrideState?: ActionsState | null): ActionsEvent | null {
    if (overrideState?.lastEvent) return overrideState.lastEvent;
    if (!envelope) return null;

    const anyEnv: any = envelope as any;
    const state = anyEnv?.context?.actionsState ?? anyEnv?.context?.actions_state ?? anyEnv?.actionsState ?? null;
...
function AppliedBanner({ event, onGoToActions, onGoToActionsRun }: { event: ActionsEvent; onGoToActions?: () => void; onGoToActionsRun?: (runId: string) => void }) {
        ...
        <div className="mx-3 mt-3 rounded-lg border bg-muted/30 px-3 py-2">
            <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium">{t('Copilot.Ask.AppliedLabel', { action: actionLabel(event.actionKey, t) })}</div>
                    <div className="mt-1 text-xs text-muted-foreground">{t('Copilot.Ask.AppliedDescription')}</div>
                ...
                <button
                    type="button"
                    className={cn('rounded p-1 text-muted-foreground hover:bg-muted/60 hover:text-foreground')}
...
const AskTab = ({ chat, copilotEnvelope, onExecuteAction, actionsState, onGoToActions, onGoToActionsRun, initialPrompt, onInitialPromptConsumed }: AskTabProps) => {
    const evt = React.useMemo(() => extractActionsEvent(copilotEnvelope, actionsState), [copilotEnvelope, actionsState]);

    return (
        <div className="flex h-full min-h-0 flex-col">
            {evt ? <AppliedBanner event={evt} onGoToActions={onGoToActions} onGoToActionsRun={onGoToActionsRun} /> : null}
