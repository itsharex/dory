    ...
    return (
        <button
            ...
            className={`flex w-full items-start gap-3 rounded-lg border bg-background px-4 py-3 text-left transition ${
                disabled ? 'cursor-not-allowed opacity-60' : 'hover:border-primary/40 hover:bg-muted/40'
            }`}
            disabled={disabled}
        >
            <span className="flex size-5 items-center justify-center text-muted-foreground">{Icon ? <Icon className="h-4 w-4 text-violet-400" /> : item.icon}</span>
            <span className="flex flex-col gap-1">
                <span className="text-sm font-medium text-foreground">{item.title}</span>
                <span className="text-xs text-muted-foreground">{item.description}</span>
                {disabled && item.reason ? <span className="text-[11px] text-amber-600">{item.reason}</span> : null}
            </span>
        </button>
    );
}
