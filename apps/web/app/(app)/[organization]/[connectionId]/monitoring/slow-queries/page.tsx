...
import { useQueryInsightsSlowQueriesHook } from '../hooks/use-monitoring';
import { useLocale, useTranslations } from 'next-intl';

const TABLE_MAX_HEIGHT = 'calc(100vh - 290px)';

export default function QueryInsightsSlowQueriesPage() {
    const t = useTranslations('Monitoring');
    const locale = useLocale();
    const filters = useQueryInsightsFiltersValue();
    const [pagination, setPagination] = useQueryInsightsPagination('slow');

    const rowsResult = useQueryInsightsSlowQueriesHook(filters, pagination);
    const rows = rowsResult.rows ?? [];
    const total = rowsResult.total ?? 0;

    const [sorting, setSorting] = React.useState<SortingState>([{ id: 'durationMs', desc: true }]);
        ...
        <section className="flex flex-col gap-4 rounded-2xl text-sm">
            ...
            <div className="w-full rounded-xl border bg-card overflow-hidden">
                <StickyDataTable<QueryInsightsRow>
                    ...
                    emptyText={t('Empty.NoSlowQueries')}
                    minBodyHeight="360px"
